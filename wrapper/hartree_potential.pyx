...
def l0(SOrbitals orbs, np.ndarray[np.double_t, ndim=1] uh = None, int order=3) -> np.ndarray:
    if uh is None:
    ...
    hartree_potential_l0(orbs.cdata, &uh[0], &uh[0], &f[0], order)
...
def wf_l0(SWavefunc wf, np.ndarray[np.double_t, ndim=1] uh = None) -> np.ndarray:
    ...
    hartree_potential_wf_l0(wf.cdata, &uh[0], &f[0])
...
def l1(SOrbitals orbs, np.ndarray[np.double_t, ndim=1] uh = None) -> np.ndarray:
    ...
    cdef np.ndarray[np.double_t, ndim=1] f = np.ndarray((orbs.cdata.wf[0].grid.n[0]), np.double)
    hartree_potential_l1(orbs.cdata, &uh[0], &uh[0], &f[0])
...
def l2(SOrbitals orbs, np.ndarray[np.double_t, ndim=1] uh = None) -> np.ndarray:
    ...
    cdef np.ndarray[np.double_t, ndim=1] f = np.ndarray((orbs.cdata.wf[0].grid.n[0]), np.double)
    hartree_potential_l2(orbs.cdata, &uh[0], &uh[0], &f[0])
