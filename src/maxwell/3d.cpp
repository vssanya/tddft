...
void maxwell::Workspace3D::prop_E_with_abs(Field3D& f, double dt, Array3D<double>& sigma, double labs, double sigma_abs) {
	double ksi_x = C_au * dt / grid.d[0];
	double ksi_y = C_au * dt / grid.d[1];
	double ksi_z = C_au * dt / grid.d[2];
		...
		for (int iy=0; iy<grid.n[1]-1; iy++) {
			for (int ix=0; ix<grid.n[0]-1; ix++) {
				...
				if (nu != 0.0) {
					...
					nu = 1.0;
					f.Hz(ix, iy, iz) = f.Hz(ix, iy, iz)*(1 - nu)/(1 + nu) + (f.Bz(ix, iy, iz) - Bl[2])/(1 + nu);
				}
			}
		}
	}
