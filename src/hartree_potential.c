...
double F_first(double[2] F, int l, sphere_grid_t const* grid, double f[grid->n[iR]]) {
	double const dr = grid->dr

	F[0] = 0;
	F[1] = 0;

	for (int ir = 0; ir < grid->n[iR]; ++ir) {
		double const r = sp_grid_r(grid, ir);
 ...
 * \param[in,out] F
 * */
double F_next(double[2] F, int l, int ir, sh_grid_t const* grid, double f[grid->n[iR]]) {
	double const dr = grid->dr
...
void hartree_potential_l0(ks_orbitals_t const* orbs, double U[orbs->grid->n[iR]]) {
	...
	for (int ie = 0; ie < orbs->ne; ++ie) {
		for (int il = 0; il < grid->n[iL]; ++il) {
			for (int ir = 0; ir < grid->n[iR]; ++ir) {
				f[ir] += swf_get_abs_2(orbs->wf[ie], ir, il)
			}
		}
	}
